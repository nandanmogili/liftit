...
import {
  ...
  DoorOpen, Flame, Home, ImagePlus, Loader2, LogOut, Plus, Search, Trash2, UserRound, Users,
} from "lucide-react";
...
type Profile = { id: string; username: string; avatar_path: string | null; created_at: string };
...
function UserAvatar({ name, large = false }: { name: string; large?: boolean }) {
  return <Avatar className={cn(large ? "size-24" : "size-11", "ring-2 ring-white/10")}><AvatarFallback className={cn("bg-[#fb7185] font-bold text-[#3d1018]", large && "text-2xl")}>{initials(name)}</AvatarFallback></Avatar>;
}
...
function Header({ eyebrow, title, username }: { eyebrow: string; title: string; username: string }) {
  return <header className="mb-7 flex items-center justify-between"><div><p className="mb-1 text-xs font-bold uppercase tracking-[.16em] text-lime">{eyebrow}</p><h1 className="text-[30px] font-black leading-none tracking-[-.045em] sm:text-4xl">{title}</h1></div><UserAvatar name={username} /></header>;
}
...
function HomeView({ username, workouts, groups, feed, profiles, onGroups }: { username: string; workouts: Workout[]; groups: Group[]; feed: Workout[]; profiles: Map<string, Profile>; onGroups: () => void }) {
...
function CalendarView({ username, workouts }: { username: string; workouts: Workout[] }) {
  ...
  return <><Header eyebrow="Your consistency" title="Calendar" username={username} /><div className="mb-5 grid grid-cols-3 gap-2 sm:gap-3">{[[monthlyCount, "this month"], [streak.count, "day streak"], [workouts.length, "this year"]].map(([value, label]) => <div key={label} className="rounded-2xl border border-white/8 bg-panel p-3.5 sm:p-4"><p className="text-2xl font-black">{value}</p><p className="mt-1 text-xs text-white/42">{label}</p></div>)}</div><div className="mb-3 flex justify-end gap-1"><button onClick={() => setMode("month")} className={cn("rounded-lg px-3 py-1.5 text-xs font-bold", mode === "month" ? "bg-white text-ink" : "bg-white/6 text-white/45")}>Month</button><button onClick={() => setMode("year")} className={cn("rounded-lg px-3 py-1.5 text-xs font-bold", mode === "year" ? "bg-white text-ink" : "bg-white/6 text-white/45")}>Year</button></div>{mode === "month" ? <section className="rounded-[26px] border border-white/8 bg-panel p-4 sm:p-6"><div className="mb-6 flex items-center justify-between"><Button disabled={month === 0} onClick={() => setMonth(value => value - 1)} variant="ghost" size="icon-sm" className="text-white/55"><ChevronLeft /></Button><h2 className="text-lg font-extrabold">{new Date(year, month).toLocaleDateString(undefined, { month: "long", year: "numeric" })}</h2><Button disabled={month === 11} onClick={() => setMonth(value => value + 1)} variant="ghost" size="icon-sm" className="text-white/55"><ChevronRight /></Button></div><div className="grid grid-cols-7 gap-1.5 text-center sm:gap-2">{["M", "T", "W", "T", "F", "S", "S"].map((day, index) => <div key={`${day}-${index}`} className="pb-2 text-xs font-bold text-white/34">{day}</div>)}{Array.from({ length: offset }).map((_, index) => <div key={`blank-${index}`} />)}{Array.from({ length: daysInMonth }, (_, index) => index + 1).map(day => { const key = dateKey(new Date(year, month, day)); const worked = workoutDates.has(key); return <div key={day} className={cn("grid aspect-square place-items-center rounded-xl text-sm font-semibold", worked && "bg-lime font-extrabold text-ink", streak.dates.has(key) && "ring-2 ring-[#fb7185] ring-offset-2 ring-offset-[#181e19]")}>{day}</div>; })}</div><div className="mt-6 flex items-center justify-center gap-5 text-xs text-white/45"><span className="flex items-center gap-2"><i className="size-2.5 rounded-sm bg-lime" />Workout</span><span className="flex items-center gap-2"><i className="size-2.5 rounded-sm border-2 border-[#fb7185]" />Current streak</span></div></section> : <section className="grid grid-cols-2 gap-3 rounded-[26px] border border-white/8 bg-panel p-4 sm:grid-cols-3 sm:p-6">{Array.from({ length: 12 }, (_, monthIndex) => { const count = workouts.filter(workout => { const d = new Date(`${workout.workout_date}T12:00:00`); return d.getFullYear() === year && d.getMonth() === monthIndex; }).length; return <button key={monthIndex} onClick={() => { setMonth(monthIndex); setMode("month"); }} className="rounded-2xl bg-white/[.035] p-4 text-left"><p className="text-sm font-bold">{new Date(year, monthIndex).toLocaleDateString(undefined, { month: "short" })}</p><p className="mt-2 text-2xl font-black text-lime">{count}</p><p className="text-xs text-white/35">workouts</p></button>; })}</section>}</>;
}

function GroupsView({ username, currentUserId, groups, selectedId, setSelectedId, members, onManage, onChanged }: { username: string; currentUserId: string; groups: Group[]; selectedId: string | null; setSelectedId: (id: string) => void; members: MemberProgress[]; onManage: () => void; onChanged: () => Promise<void> }) {
  ...
  return <>
    <Header eyebrow={`${groups.length} active group${groups.length === 1 ? "" : "s"}`} title={selected.name} username={username} />
    <div className="mb-5 flex gap-2 overflow-x-auto pb-1 scrollbar-none">{groups.map(group => <button key={group.id} onClick={() => { setSelectedId(group.id); setActionError(""); }} className={cn("shrink-0 rounded-full px-4 py-2 text-sm font-bold", selected.id === group.id ? "bg-white text-ink" : "bg-white/7 text-white/48")}>{group.name}</button>)}<button onClick={onManage} className="grid size-9 shrink-0 place-items-center rounded-full border border-dashed border-white/20 text-white/45"><Plus className="size-4" /></button></div>
    ...
    <section className="mt-7"><div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-extrabold">This week</h2><span className="text-xs text-white/40">Mon–Sun</span></div><div className="space-y-3">{members.map(member => <div key={member.profile.id} className="rounded-[20px] border border-white/8 bg-panel p-4"><div className="mb-3 flex items-center gap-3"><UserAvatar name={member.profile.username} /><div className="flex-1 font-bold">{member.profile.username}{member.profile.id === currentUserId && <span className="ml-2 text-xs font-medium text-white/35">you</span>}</div><span className={cn("text-sm font-black", member.count >= selected.weekly_quota ? "text-lime" : "text-white/66")}>{member.count} / {selected.weekly_quota} {member.count >= selected.weekly_quota && <Check className="ml-1 inline size-4" />}</span></div><Progress value={Math.min(100, (member.count / selected.weekly_quota) * 100)} className="h-2 bg-white/8 [&_[data-slot=progress-indicator]]:bg-lime" /></div>)}</div></section>
    ...
    <Dialog open={membersOpen} onOpenChange={setMembersOpen}><DialogContent className="rounded-[26px] border-white/10 bg-[#181e19] p-5 text-white sm:p-6"><DialogHeader><DialogTitle className="text-2xl font-black">{selected.name} members</DialogTitle><DialogDescription className="text-white/42">{members.length} of 10 spots filled</DialogDescription></DialogHeader><div className="mt-2 space-y-2">{members.map(member => <div key={member.profile.id} className="flex items-center gap-3 rounded-2xl bg-white/5 p-3"><UserAvatar name={member.profile.username} /><div className="min-w-0 flex-1"><p className="truncate font-bold">{member.profile.username}{member.profile.id === currentUserId && <span className="ml-2 text-xs font-medium text-white/35">you</span>}</p><p className="text-xs capitalize text-white/40">{member.role}</p></div></div>)}</div></DialogContent></Dialog>
  </>;
}

function ProfileView({ profile, workoutCount, streak, groupCount }: { profile: Profile; workoutCount: number; streak: number; groupCount: number }) {
...
export default function LiftItApp() {
  ...
  const refresh = useCallback(async (preferredGroupId?: string) => {
    ...
    if (!profileData) { const fallback = String(user.user_metadata.username || user.email?.split("@")[0] || "member").slice(0, 24); const inserted = await supabase.from("profiles").upsert({ id: user.id, username: fallback }).select("*").single(); profileData = inserted.data; }
    ...
    if (!groupIds.length) { setGroups([]); setFeed([]); setGroupWorkouts([]); setProfiles(new Map(profileData ? [[profileData.id, profileData as Profile]] : [])); setSelectedGroupId(null); setLoading(false); return; }
    ...
    const profileMap = new Map<string, Profile>(((profileRows ?? []) as Profile[]).map(item => [item.id, item])); setProfiles(profileMap); setGroups(groupList); setMemberships(membershipRows);
  ...
  return <main className="min-h-screen bg-ink text-white"><div className="mx-auto flex min-h-screen max-w-[1180px]"><DesktopNav view={view} setView={setView} onLog={() => setLogOpen(true)} groupCount={groups.length} /><div className="min-w-0 flex-1"><div className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-white/8 bg-ink/88 px-5 backdrop-blur-xl md:hidden"><Brand /><span className="text-xs font-bold uppercase tracking-[.13em] text-white/35">{pageTitle}</span></div><div className="mx-auto w-full max-w-[720px] px-4 pb-28 pt-7 sm:px-7 md:pb-16 md:pt-10">{view === "home" && <HomeView username={profile.username} workouts={workouts} groups={groups} feed={feed} profiles={profiles} onGroups={() => setView("groups")} />}{view === "calendar" && <CalendarView username={profile.username} workouts={workouts} />}{view === "groups" && <GroupsView username={profile.username} currentUserId={profile.id} groups={groups} selectedId={selectedGroupId} setSelectedId={setSelectedGroupId} members={memberProgress} onManage={() => setGroupOpen(true)} onChanged={() => refresh()} />}{view === "profile" && <ProfileView profile={profile} workoutCount={workouts.length} streak={streak} groupCount={groups.length} />}</div></div></div><AppNav view={view} setView={setView} onLog={() => setLogOpen(true)} /><LogDialog open={logOpen} onOpenChange={setLogOpen} onLogged={() => refresh(selectedGroupId ?? undefined)} /><GroupDialog open={groupOpen} onOpenChange={setGroupOpen} discoverGroups={discoverGroups.filter(group => !groups.some(own => own.id === group.id))} onChanged={refresh} /></main>;
}
