...
import {
  ...
  Copy, Crown, DoorOpen, Flame, Home, ImagePlus, Loader2, LogOut, Plus, RefreshCw, Search, Settings, Trash2, Trophy, UserMinus, UserRound, Users,
} from "lucide-react";
...
type Profile = { id: string; username: string; avatar_path: string | null; avatar_position_x: number; avatar_position_y: number; avatar_zoom: number; weight_tracking_enabled?: boolean; strength_tracking_enabled?: boolean; preferred_weight_unit?: "lb" | "kg"; created_at: string; avatar_url?: string };
...
type StrengthAchievement = { id: string; user_id: string; lift_key: string; lift_label: string; lift_weight_kg: number; achieved_week: string; created_at: string };
...
function HomeView({ username, avatarUrl, avatarPositionX, avatarPositionY, avatarZoom, currentUserId, workouts, groups, feed, achievements, profiles, reactions, onProfile, onGroups, onDeleted, onReactionChanged }: { username: string; avatarUrl?: string; avatarPositionX?: number; avatarPositionY?: number; avatarZoom?: number; currentUserId: string; workouts: Workout[]; groups: Group[]; feed: Workout[]; achievements: StrengthAchievement[]; profiles: Map<string, Profile>; reactions: Reaction[]; onProfile: () => void; onGroups: () => void; onDeleted: () => Promise<void>; onReactionChanged: () => Promise<void> }) {
  ...
  const activityItems = [...feed.map(item => ({ kind: "workout" as const, item, createdAt: item.created_at })), ...achievements.map(item => ({ kind: "achievement" as const, item, createdAt: item.created_at }))].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 20);
...
<p className="relative z-10 mt-5 text-sm text-white/52">{quota ? weeklyCount >= quota ? "You hit your highest group quota this week." : `${quota - weeklyCount} workout${quota - weeklyCount === 1 ? "" : "s"} to hit your highest group quota.` : "Join or create a group to set your weekly quota."}</p></section><section className="mt-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-extrabold tracking-tight">Group activity</h2></div>{groups.length === 0 ? <EmptyState icon={Users} title="No group activity yet" detail="Create or join a group to see your friends’ real workout posts here." action="Find or create a group" onAction={onGroups} /> : activityItems.length === 0 ? <EmptyState icon={Activity} title="Quiet week so far" detail="Nobody in your groups has logged a workout or reached a strength benchmark this week yet." /> : <div className="space-y-3">{activityItems.map(activity => { const author = profiles.get(activity.item.user_id); if (activity.kind === "achievement") { const achievement = activity.item; return <article key={`achievement-${achievement.id}`} className="mx-1 max-w-[600px] overflow-hidden rounded-[20px] border border-[#9b72cf]/25 bg-[linear-gradient(135deg,rgba(155,114,207,.12),rgba(24,30,25,.96))] sm:mx-auto"><div className="flex items-center gap-3 p-4"><UserAvatar name={author?.username ?? "Member"} avatarUrl={author?.avatar_url} positionX={author?.avatar_position_x} positionY={author?.avatar_position_y} zoom={author?.avatar_zoom} /><div className="min-w-0 flex-1"><p className="font-bold"><span className="text-[#c5a4ef]">{author?.username ?? "Member"}</span> hit a Dominant benchmark</p><p className="mt-0.5 text-sm text-white/58">{achievement.lift_label}</p><p className="mt-1 text-xs text-white/35">{relativeTime(achievement.created_at)}</p></div><span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-[#9b72cf]/18 text-[#c5a4ef]"><Trophy className="size-5" /></span></div></article>; } const workout = activity.item; return <article key={workout.id} className="mx-1 max-w-[600px] overflow-hidden rounded-[20px] border border-white/8 bg-panel sm:mx-auto"><div className="flex gap-3 p-3.5 sm:p-4"><UserAvatar name={author?.username ?? "Member"} avatarUrl={author?.avatar_url} positionX={author?.avatar_position_x} positionY={author?.avatar_position_y} zoom={author?.avatar_zoom} /><div className="flex-1"><p className="font-bold">{author?.username ?? "Member"} logged {workout.workout_type}</p><p className="mt-0.5 text-xs text-white/42">{relativeTime(workout.created_at)}</p></div>{workout.user_id === currentUserId && <button onClick={() => deleteWorkout(workout)} disabled={deletingId === workout.id} aria-label="Delete workout" className="grid size-9 shrink-0 place-items-center rounded-xl text-white/35 transition hover:bg-red-400/10 hover:text-red-300">{deletingId === workout.id ? <Loader2 className="size-4 animate-spin" /> : <Trash2 className="size-4" />}</button>}</div>{workout.proof_url && <div className="relative mx-3 overflow-hidden rounded-xl bg-black/35 sm:mx-4"><img src={workout.proof_url} alt="" aria-hidden="true" className="absolute inset-0 size-full scale-110 object-cover opacity-25 blur-2xl" /><img src={workout.proof_url} alt={`${workout.workout_type} proof`} className="relative mx-auto block max-h-[420px] max-w-full object-contain sm:max-h-[520px]" /></div>}
...
function GroupSettingsDialog({ group, open, onOpenChange, onChanged, onManageMembers }: { group: Group; open: boolean; onOpenChange: (open: boolean) => void; onChanged: () => Promise<void>; onManageMembers: () => void }) {
  ...
  useEffect(() => {
    if (!open) return;
    setName(group.name); setDescription(group.description); setQuota(group.weekly_quota); setPassword(""); setMessage(""); setIsError(false); setDeleteConfirmOpen(false); setDeleteConfirmation(""); setDeleteError("");
  ...
  async function deleteGroup() {
    if (deleteConfirmation.trim() !== group.name) return;
    const supabase = getSupabase(); if (!supabase) return;
    setSaving(true); setDeleteError("");
    const { error } = await supabase.from("groups").delete().eq("id", group.id);
    if (error) { setSaving(false); setDeleteError(error.message); return; }
    setDeleteConfirmOpen(false); onOpenChange(false); await onChanged();
  }

  return <><Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] overflow-y-auto rounded-[26px] border-white/10 bg-[#181e19] p-5 text-white sm:max-w-lg sm:p-6"><DialogHeader><DialogTitle className="flex items-center gap-2 text-2xl font-black"><Settings className="size-5 text-lime" />Group settings</DialogTitle><DialogDescription className="text-white/42">Only the group owner can make these changes.</DialogDescription></DialogHeader><div className="space-y-5"><div className="space-y-4 rounded-2xl border border-white/8 bg-white/[.025] p-4"><div><label htmlFor="settings-name" className="mb-2 block text-sm font-bold">Group name</label><Input id="settings-name" value={name} onChange={event => setName(event.target.value)} maxLength={50} className="h-11 border-white/10 bg-white/5" /></div><div><label htmlFor="settings-description" className="mb-2 block text-sm font-bold">Description</label><Textarea id="settings-description" value={description} onChange={event => setDescription(event.target.value)} maxLength={180} className="min-h-20 resize-none border-white/10 bg-white/5" /></div><div><label htmlFor="settings-quota" className="mb-2 block text-sm font-bold">Weekly quota</label><Input id="settings-quota" type="number" min={1} max={14} value={quota} onChange={event => setQuota(Number(event.target.value))} className="h-11 border-white/10 bg-white/5" /></div><div><label htmlFor="settings-password" className="mb-2 block text-sm font-bold">New group password <span className="font-normal text-white/35">optional</span></label><Input id="settings-password" type="password" value={password} onChange={event => setPassword(event.target.value)} minLength={4} placeholder="Leave blank to keep current password" className="h-11 border-white/10 bg-white/5" /></div><Button onClick={saveSettings} disabled={saving || regenerating} className="h-11 w-full rounded-xl bg-lime font-black text-ink hover:bg-[#d6ff6a]">{saving && <Loader2 className="animate-spin" />}Save changes</Button></div><div className="rounded-2xl border border-white/8 bg-white/[.025] p-4"><p className="font-extrabold">Invite code</p><p className="mt-1 text-xs text-white/40">Anyone with this code can join without the password.</p><div className="mt-3 flex items-center gap-2"><code className="min-w-0 flex-1 select-all truncate rounded-xl bg-black/20 px-3 py-2.5 text-sm font-bold tracking-wider text-lime">{group.invite_code}</code><Button onClick={copyInviteCode} variant="outline" size="icon" className="shrink-0 rounded-xl border-white/10 bg-white/5 text-white hover:bg-white/10 hover:text-white" aria-label="Copy invite code"><Copy className="size-4" /></Button><Button onClick={regenerateInviteCode} disabled={regenerating || saving} variant="outline" size="icon" className="shrink-0 rounded-xl border-white/10 bg-white/5 text-white hover:bg-white/10 hover:text-white" aria-label="Regenerate invite code">{regenerating ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />}</Button></div></div><Button onClick={() => { onOpenChange(false); onManageMembers(); }} variant="outline" className="h-11 w-full rounded-xl border-white/10 bg-white/5 text-white hover:bg-white/10 hover:text-white"><Users className="size-4" />Manage members</Button>{message && <p className={cn("rounded-xl p-3 text-sm", isError ? "bg-red-400/10 text-red-200" : "bg-lime/10 text-lime")}>{message}</p>}<div className="rounded-2xl border border-red-400/15 bg-red-400/[.045] p-4"><p className="font-extrabold text-red-200">Danger zone</p><p className="mt-1 text-xs leading-5 text-white/40">Deleting the group removes it for everyone. Members keep their personal workout history.</p><Button onClick={() => { setDeleteConfirmation(""); setDeleteError(""); setDeleteConfirmOpen(true); }} disabled={saving || regenerating} variant="outline" className="mt-3 h-10 w-full rounded-xl border-red-400/20 bg-red-400/8 text-red-200 hover:bg-red-400/15 hover:text-red-100"><Trash2 className="size-4" />Delete group</Button></div></div></DialogContent></Dialog><Dialog open={deleteConfirmOpen} onOpenChange={open => { if (!saving) { setDeleteConfirmOpen(open); if (!open) { setDeleteConfirmation(""); setDeleteError(""); } } }}><DialogContent className="rounded-[26px] border-red-400/20 bg-[#181e19] p-5 text-white sm:max-w-md sm:p-6"><DialogHeader><DialogTitle className="text-2xl font-black text-red-200">Delete {group.name}?</DialogTitle><DialogDescription className="leading-6 text-white/45">This permanently removes the group for every member. Personal workout history will not be deleted.</DialogDescription></DialogHeader><div><label htmlFor="delete-group-confirmation" className="mb-2 block text-sm font-bold">Type <span className="select-all text-red-200">{group.name}</span> to confirm</label><Input id="delete-group-confirmation" value={deleteConfirmation} onChange={event => { setDeleteConfirmation(event.target.value); setDeleteError(""); }} autoComplete="off" className="h-12 border-red-400/20 bg-white/5" /></div>{deleteError && <p className="rounded-xl bg-red-400/10 p-3 text-sm text-red-200">{deleteError}</p>}<div className="grid grid-cols-2 gap-2"><Button onClick={() => setDeleteConfirmOpen(false)} disabled={saving} variant="outline" className="h-11 rounded-xl border-white/10 bg-white/5 text-white hover:bg-white/10 hover:text-white">Cancel</Button><Button onClick={deleteGroup} disabled={saving || deleteConfirmation.trim() !== group.name} className="h-11 rounded-xl bg-red-500 font-black text-white hover:bg-red-400 disabled:bg-red-500/25 disabled:text-white/35">{saving ? <Loader2 className="animate-spin" /> : <Trash2 className="size-4" />}Delete forever</Button></div></DialogContent></Dialog></>;
}

function PublicProfileDialog({ member, group, workouts, open, onOpenChange }: { member: MemberProgress | null; group: Group; workouts: Workout[]; open: boolean; onOpenChange: (open: boolean) => void }) {
  if (!member) return null;
  const memberWorkouts = workouts.filter(workout => workout.user_id === member.profile.id);
  const monthKey = dateKey(new Date()).slice(0, 7);
  const monthlyCount = memberWorkouts.filter(workout => workout.workout_date.startsWith(monthKey)).length;
  const streak = calculateStreak(memberWorkouts).count;
  const completed = member.count >= group.weekly_quota;
  const memberSince = new Date(member.profile.created_at).toLocaleDateString(undefined, { month: "long", year: "numeric" });
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="rounded-[28px] border-white/10 bg-[#181e19] p-5 text-white sm:p-6"><DialogHeader><DialogTitle className="sr-only">{member.profile.username}&apos;s profile</DialogTitle><DialogDescription className="sr-only">Public workout information shared with group members.</DialogDescription></DialogHeader><div className="text-center"><div className="mx-auto w-fit"><UserAvatar name={member.profile.username} avatarUrl={member.profile.avatar_url} positionX={member.profile.avatar_position_x} positionY={member.profile.avatar_position_y} zoom={member.profile.avatar_zoom} large /></div><h2 className="mt-4 text-2xl font-black">@{member.profile.username}</h2><div className="mt-1 flex items-center justify-center gap-1.5 text-sm text-white/40">{member.role === "owner" && <Crown className="size-4 fill-[#F6C453]/20 text-[#F6C453]" />}<span>{member.role === "owner" ? "Group owner" : `Member of ${group.name}`}</span></div><p className="mt-1 text-xs text-white/28">Member since {memberSince}</p></div><div className="mt-6 grid grid-cols-3 divide-x divide-white/8 rounded-2xl bg-white/[.035] py-4 text-center"><div><p className="text-2xl font-black">{memberWorkouts.length}</p><p className="text-xs text-white/40">this year</p></div><div><p className="text-2xl font-black">{monthlyCount}</p><p className="text-xs text-white/40">this month</p></div><div><p className="text-2xl font-black">{streak}</p><p className="text-xs text-white/40">day streak</p></div></div><div className="mt-4 rounded-2xl border border-white/8 bg-white/[.025] p-4"><div className="mb-3 flex items-center justify-between"><div className="text-left"><p className="font-extrabold">Weekly progress</p><p className="text-xs text-white/35">Monday–Sunday</p></div><p className={cn("font-black", completed ? "text-[#F6C453]" : "text-white/65")}>{member.count} / {group.weekly_quota} {completed && <Check className="ml-1 inline size-4" />}</p></div><Progress value={Math.min(100, (member.count / group.weekly_quota) * 100)} className={cn("h-2 bg-white/8", completed ? "[&_[data-slot=progress-indicator]]:bg-[#F6C453] [&_[data-slot=progress-indicator]]:shadow-[0_0_12px_rgba(246,196,83,.28)]" : "[&_[data-slot=progress-indicator]]:bg-lime")} /></div><p className="mt-4 text-center text-xs leading-5 text-white/28">Weight tracking, email, and other private account information are never shared.</p></DialogContent></Dialog>;
}
...
function GroupsView({ username, avatarUrl, avatarPositionX, avatarPositionY, avatarZoom, currentUserId, groups, selectedId, setSelectedId, members, workouts, onProfile, onManage, onChanged }: { username: string; avatarUrl?: string; avatarPositionX?: number; avatarPositionY?: number; avatarZoom?: number; currentUserId: string; groups: Group[]; selectedId: string | null; setSelectedId: (id: string) => void; members: MemberProgress[]; workouts: Workout[]; onProfile: () => void; onManage: () => void; onChanged: () => Promise<void> }) {
  ...
  function openMemberProfile(member: MemberProgress) {
  ...
  return <>
    ...
    <div className="mb-5 flex gap-2 overflow-x-auto pb-1 scrollbar-none">{groups.map(group => <button key={group.id} onClick={() => { setSelectedId(group.id); setSection("progress"); setActionError(""); setProfileMember(null); }} className={cn("shrink-0 rounded-full px-4 py-2 text-sm font-bold", selected.id === group.id ? "bg-white text-ink" : "bg-white/7 text-white/48")}>{group.name}</button>)}<button onClick={onManage} className="grid size-9 shrink-0 place-items-center rounded-full border border-dashed border-white/20 text-white/45"><Plus className="size-4" /></button></div>
    ...
    <Dialog open={membersOpen} onOpenChange={setMembersOpen}>
      <DialogContent className="rounded-[26px] border-white/10 bg-[#181e19] p-5 text-white sm:p-6">
        <DialogHeader>
          ...
          <DialogDescription className="text-white/42">{members.length} of 20 spots filled{isOwner ? " · You can remove members" : ""}</DialogDescription>
        </DialogHeader>
        <div className="mt-2 space-y-2">{members.map(member => <div key={member.profile.id} className="flex items-center gap-2 rounded-2xl bg-white/5 p-2">
          <button type="button" onClick={() => openMemberProfile(member)} className="flex min-w-0 flex-1 items-center gap-3 rounded-xl p-1 text-left transition hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime/60"><UserAvatar name={member.profile.username} avatarUrl={member.profile.avatar_url} positionX={member.profile.avatar_position_x} positionY={member.profile.avatar_position_y} zoom={member.profile.avatar_zoom} />
          <span className="min-w-0 flex-1"><span className="block truncate font-bold">{member.profile.username}{member.profile.id === currentUserId && <span className="ml-2 text-xs font-medium text-white/35">you</span>}</span>{member.role === "owner" ? <span className="flex items-center gap-1 text-xs font-bold text-amber-300"><Crown className="size-3.5 fill-amber-300/20" />Owner</span> : <span className="text-xs capitalize text-white/40">Member</span>}</span></button>
    ...
    <PublicProfileDialog member={profileMember} group={selected} workouts={selectedWorkouts} open={Boolean(profileMember)} onOpenChange={open => { if (!open) setProfileMember(null); }} />
...
function ProfileView({ profile, workoutCount, streak, groupCount, onChanged }: { profile: Profile; workoutCount: number; streak: number; groupCount: number; onChanged: () => Promise<void> }) {
  ...
  useEffect(() => { if (settingsOpen) { setUsernameDraft(profile.username); setSettingsMessage(""); setSettingsError(""); } }, [settingsOpen, profile.username]);
  ...
  function choosePhoto(file?: File) {
    ...
    setPhotoError(""); setSettingsOpen(false); setCropFile(file); setCropPreview(URL.createObjectURL(file)); setCropX(50); setCropY(50); setCropZoom(100); setCropOpen(true);
  }
  ...
  async function saveUsername() {
    ...
    if (nextUsername.length < 3 || nextUsername.length > 24) { setSettingsError("Username must be between 3 and 24 characters."); return; }
    if (nextUsername === profile.username) { setSettingsMessage("Your username is already up to date."); setSettingsError(""); return; }
    const supabase = getSupabase(); if (!supabase) return;
    setSettingsSaving(true); setSettingsMessage(""); setSettingsError("");
    const duplicate = await supabase.from("profiles").select("id").ilike("username", nextUsername).neq("id", profile.id).limit(1);
    if (duplicate.error) { setSettingsSaving(false); setSettingsError(duplicate.error.message); return; }
    if (duplicate.data?.length) { setSettingsSaving(false); setSettingsError("That username is already taken."); return; }
    const update = await supabase.from("profiles").update({ username: nextUsername }).eq("id", profile.id);
    if (update.error) { setSettingsSaving(false); setSettingsError(update.error.code === "23505" ? "That username is already taken." : update.error.message); return; }
    await supabase.auth.updateUser({ data: { username: nextUsername } });
    await onChanged(); setUsernameDraft(nextUsername); setSettingsSaving(false); setSettingsMessage("Username updated.");
  }
  async function deleteAccount() {
    if (accountConfirmation.trim() !== profile.username) return;
    const supabase = getSupabase(); if (!supabase) return;
    setDeletingAccount(true); setSettingsError("");
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) { setDeletingAccount(false); setSettingsError("Your session has expired. Log in again before deleting your account."); return; }
    const response = await fetch("/api/account", { method: "DELETE", headers: { "authorization": `Bearer ${session.access_token}`, "content-type": "application/json" }, body: JSON.stringify({ confirmation: accountConfirmation }) });
    const result = await response.json().catch(() => ({ error: "Could not delete the account." })) as { error?: string };
    if (!response.ok) { setDeletingAccount(false); setSettingsError(result.error ?? "Could not delete the account."); return; }
    await supabase.auth.signOut({ scope: "local" });
    window.location.href = "/login";
  }
  const memberSince = new Date(profile.created_at).toLocaleDateString(undefined, { month: "long", year: "numeric" });
  return <><Header eyebrow="Your profile" title={profile.username} username={profile.username} avatarUrl={profile.avatar_url} avatarPositionX={profile.avatar_position_x} avatarPositionY={profile.avatar_position_y} avatarZoom={profile.avatar_zoom} /><section className="rounded-[28px] border border-white/8 bg-panel p-6 text-center"><div className="mx-auto w-fit"><UserAvatar name={profile.username} avatarUrl={profile.avatar_url} positionX={profile.avatar_position_x} positionY={profile.avatar_position_y} zoom={profile.avatar_zoom} large /></div><h2 className="mt-4 text-2xl font-black">@{profile.username}</h2><p className="mt-1 text-sm text-white/40">Member since {memberSince}</p><div className="mt-6 grid grid-cols-3 divide-x divide-white/8 rounded-2xl bg-white/[.035] py-4"><div><p className="text-2xl font-black">{workoutCount}</p><p className="text-xs text-white/40">this year</p></div><div><p className="text-2xl font-black">{streak}</p><p className="text-xs text-white/40">day streak</p></div><div><p className="text-2xl font-black">{groupCount}</p><p className="text-xs text-white/40">groups</p></div></div></section><button onClick={() => setSettingsOpen(true)} className="mt-5 flex w-full items-center gap-3 rounded-2xl border border-white/8 bg-panel p-4 text-left font-bold text-white/70 transition hover:bg-white/[.045]"><span className="grid size-9 place-items-center rounded-xl bg-white/6"><Settings className="size-4" /></span><span className="flex-1">Profile settings</span><ChevronRight className="size-4 text-white/25" /></button><WeightTracker userId={profile.id} initialEnabled={profile.weight_tracking_enabled ?? false} initialUnit={profile.preferred_weight_unit ?? "lb"} onProfileChanged={onChanged} /><StrengthTracker userId={profile.id} initialEnabled={profile.strength_tracking_enabled ?? false} initialUnit={profile.preferred_weight_unit ?? "lb"} onProfileChanged={onChanged} /><button onClick={signOut} className="mt-5 flex w-full items-center gap-3 rounded-2xl border border-white/8 bg-panel p-4 text-left font-bold text-white/65"><span className="grid size-9 place-items-center rounded-xl bg-white/6"><LogOut className="size-4" /></span>Log out</button><Dialog open={settingsOpen} onOpenChange={open => { if (!settingsSaving && !uploading) setSettingsOpen(open); }}><DialogContent className="max-h-[92vh] overflow-y-auto rounded-[26px] border-white/10 bg-[#181e19] p-5 text-white sm:p-6"><DialogHeader><DialogTitle className="text-2xl font-black">Profile settings</DialogTitle><DialogDescription className="text-white/42">Update how your profile appears to your groups.</DialogDescription></DialogHeader><div className="space-y-5"><div className="rounded-2xl border border-white/8 bg-white/[.025] p-4"><p className="mb-3 font-extrabold">Profile picture</p><div className="flex items-center gap-4"><UserAvatar name={profile.username} avatarUrl={profile.avatar_url} positionX={profile.avatar_position_x} positionY={profile.avatar_position_y} zoom={profile.avatar_zoom} /><label htmlFor="settings-profile-photo" className="flex h-10 cursor-pointer items-center gap-2 rounded-xl bg-white/8 px-4 text-sm font-bold text-white transition hover:bg-white/12"><Camera className="size-4" />{profile.avatar_path ? "Change picture" : "Add picture"}<input id="settings-profile-photo" type="file" accept="image/*" className="sr-only" disabled={uploading} onChange={event => { choosePhoto(event.target.files?.[0]); event.currentTarget.value = ""; }} /></label></div>{photoError && <p className="mt-3 rounded-xl bg-red-400/10 p-3 text-sm text-red-200">{photoError}</p>}</div><div className="rounded-2xl border border-white/8 bg-white/[.025] p-4"><label htmlFor="settings-username" className="mb-2 block font-extrabold">Username</label><p className="mb-3 text-xs leading-5 text-white/38">Usernames must be unique and between 3 and 24 characters.</p><Input id="settings-username" value={usernameDraft} onChange={event => { setUsernameDraft(event.target.value); setSettingsError(""); setSettingsMessage(""); }} minLength={3} maxLength={24} autoComplete="username" className="h-11 border-white/10 bg-white/5" /><Button onClick={saveUsername} disabled={settingsSaving || usernameDraft.trim() === profile.username} className="mt-3 h-10 w-full rounded-xl bg-lime font-black text-ink hover:bg-[#d6ff6a]">{settingsSaving && <Loader2 className="animate-spin" />}Save username</Button></div>{settingsMessage && <p className="rounded-xl bg-lime/10 p-3 text-sm text-lime">{settingsMessage}</p>}{settingsError && <p className="rounded-xl bg-red-400/10 p-3 text-sm text-red-200">{settingsError}</p>}<div className="rounded-2xl border border-red-400/15 bg-red-400/[.045] p-4"><p className="font-extrabold text-red-200">Delete account</p><p className="mt-1 text-xs leading-5 text-white/40">Permanently deletes your workouts, weight and strength data, memberships, images, and any groups you own.</p><Button onClick={() => { setAccountConfirmation(""); setSettingsError(""); setSettingsOpen(false); setAccountDeleteOpen(true); }} variant="outline" className="mt-3 h-10 w-full rounded-xl border-red-400/20 bg-red-400/8 text-red-200 hover:bg-red-400/15 hover:text-red-100"><Trash2 className="size-4" />Delete my account</Button></div></div></DialogContent></Dialog><Dialog open={accountDeleteOpen} onOpenChange={open => { if (!deletingAccount) { setAccountDeleteOpen(open); if (!open) setAccountConfirmation(""); } }}><DialogContent className="rounded-[26px] border-red-400/20 bg-[#181e19] p-5 text-white sm:max-w-md sm:p-6"><DialogHeader><DialogTitle className="text-2xl font-black text-red-200">Delete your account?</DialogTitle><DialogDescription className="leading-6 text-white/45">This cannot be undone. Your workouts, weight and strength entries, memberships, uploaded images, and groups you own will be permanently deleted.</DialogDescription></DialogHeader><div><label htmlFor="delete-account-confirmation" className="mb-2 block text-sm font-bold">Type <span className="select-all text-red-200">{profile.username}</span> to confirm</label><Input id="delete-account-confirmation" value={accountConfirmation} onChange={event => { setAccountConfirmation(event.target.value); setSettingsError(""); }} autoComplete="off" className="h-12 border-red-400/20 bg-white/5" /></div>{settingsError && <p className="rounded-xl bg-red-400/10 p-3 text-sm text-red-200">{settingsError}</p>}<div className="grid grid-cols-2 gap-2"><Button onClick={() => setAccountDeleteOpen(false)} disabled={deletingAccount} variant="outline" className="h-11 rounded-xl border-white/10 bg-white/5 text-white hover:bg-white/10 hover:text-white">Cancel</Button><Button onClick={deleteAccount} disabled={deletingAccount || accountConfirmation.trim() !== profile.username} className="h-11 rounded-xl bg-red-500 font-black text-white hover:bg-red-400 disabled:bg-red-500/25 disabled:text-white/35">{deletingAccount ? <Loader2 className="animate-spin" /> : <Trash2 className="size-4" />}Delete forever</Button></div></DialogContent></Dialog><Dialog open={cropOpen} onOpenChange={open => { if (!open && !uploading) closeCrop(); }}><DialogContent className="rounded-[26px] border-white/10 bg-[#181e19] p-5 text-white sm:p-6"><DialogHeader><DialogTitle className="text-2xl font-black">Position your picture</DialogTitle><DialogDescription className="text-white/42">Drag the photo until it looks right inside the circle.</DialogDescription></DialogHeader><div className="flex justify-center py-4"><div className="relative size-[260px] touch-none cursor-grab overflow-hidden rounded-full border-4 border-lime/70 bg-black active:cursor-grabbing" onPointerDown={event => { event.currentTarget.setPointerCapture(event.pointerId); dragRef.current = { clientX: event.clientX, clientY: event.clientY, positionX: cropX, positionY: cropY, width: event.currentTarget.clientWidth, height: event.currentTarget.clientHeight }; }} onPointerMove={event => { const drag = dragRef.current; if (!drag) return; setCropX(Math.max(0, Math.min(100, drag.positionX - ((event.clientX - drag.clientX) / drag.width) * 100))); setCropY(Math.max(0, Math.min(100, drag.positionY - ((event.clientY - drag.clientY) / drag.height) * 100))); }} onPointerUp={() => { dragRef.current = null; }} onPointerCancel={() => { dragRef.current = null; }}>{cropPreview && <img src={cropPreview} alt="Profile crop preview" draggable={false} className="pointer-events-none size-full select-none object-cover" style={{ objectPosition: `${cropX}% ${cropY}%`, transform: `scale(${cropZoom / 100})`, transformOrigin: `${cropX}% ${cropY}%` }} />}<div className="pointer-events-none absolute inset-0 rounded-full ring-1 ring-inset ring-white/30" /></div></div><label className="mb-5 block text-sm font-bold text-white/60">Zoom <span className="float-right text-white/35">{cropZoom}%</span><input type="range" min="100" max="300" step="5" value={cropZoom} onChange={event => setCropZoom(Number(event.target.value))} className="mt-3 w-full accent-lime" /></label><div className="grid grid-cols-2 gap-2"><Button onClick={closeCrop} disabled={uploading} variant="outline" className="h-11 rounded-xl border-white/10 bg-white/5 text-white hover:bg-white/10 hover:text-white">Cancel</Button><Button onClick={saveCrop} disabled={uploading} className="h-11 rounded-xl bg-lime font-black text-ink hover:bg-[#d6ff6a]">{uploading && <Loader2 className="animate-spin" />}Save picture</Button></div></DialogContent></Dialog></>;
}
...
export default function LiftItApp() {
  ...
  const [achievements, setAchievements] = useState<StrengthAchievement[]>([]);
  ...
  const refresh = useCallback(async (preferredGroupId?: string) => {
    ...
    if (!groupIds.length) { setGroups([]); setFeed([]); setAchievements([]); setReactions([]); setGroupWorkouts([]); setProfiles(new Map(ownProfile ? [[ownProfile.id, ownProfile]] : [])); setSelectedGroupId(null); setLoading(false); return; }
    const week = easternWeekStartKey();
    const [{ data: groupRows }, { data: allMemberships }, { data: visibleWorkouts }, { data: achievementRows }] = await Promise.all([
      ...
      supabase.from("strength_achievements").select("*").eq("achieved_week", week).order("created_at", { ascending: false }).limit(50),
    ]);
    ...
    const memberSet = new Set(memberIds); const visible = ((visibleWorkouts ?? []) as Workout[]).filter(workout => memberSet.has(workout.user_id)); setGroupWorkouts(visible);
    setAchievements(((achievementRows ?? []) as StrengthAchievement[]).filter(achievement => memberSet.has(achievement.user_id)));
...
{view === "home" && <HomeView username={profile.username} avatarUrl={profile.avatar_url} avatarPositionX={profile.avatar_position_x} avatarPositionY={profile.avatar_position_y} avatarZoom={profile.avatar_zoom} currentUserId={profile.id} workouts={workouts} groups={groups} feed={feed.filter(workout => workout.workout_date >= weeklyStart)} achievements={achievements} profiles={profiles} reactions={reactions} onProfile={() => setView("profile")} onGroups={() => setView("groups")} onDeleted={() => refresh(selectedGroupId ?? undefined)} onReactionChanged={() => refresh(selectedGroupId ?? undefined)} />}
