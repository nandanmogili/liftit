...
type Profile = { id: string; username: string; avatar_path: string | null; avatar_position_x: number; avatar_position_y: number; created_at: string; avatar_url?: string };
...
function UserAvatar({ name, avatarUrl, positionX = 50, positionY = 50, large = false }: { name: string; avatarUrl?: string; positionX?: number; positionY?: number; large?: boolean }) {
  return <Avatar className={cn(large ? "size-24" : "size-11", "ring-2 ring-white/10")}>{avatarUrl && <AvatarImage src={avatarUrl} alt={`${name}'s profile picture`} className="object-cover" style={{ objectPosition: `${positionX}% ${positionY}%` }} />}<AvatarFallback className={cn("bg-[#fb7185] font-bold text-[#3d1018]", large && "text-2xl")}>{initials(name)}</AvatarFallback></Avatar>;
}
...
function Header({ eyebrow, title, username, avatarUrl, avatarPositionX, avatarPositionY }: { eyebrow: string; title: string; username: string; avatarUrl?: string; avatarPositionX?: number; avatarPositionY?: number }) {
  return <header className="mb-7 flex items-center justify-between"><div><p className="mb-1 text-xs font-bold uppercase tracking-[.16em] text-lime">{eyebrow}</p><h1 className="text-[30px] font-black leading-none tracking-[-.045em] sm:text-4xl">{title}</h1></div><UserAvatar name={username} avatarUrl={avatarUrl} positionX={avatarPositionX} positionY={avatarPositionY} /></header>;
}
...
function HomeView({ username, avatarUrl, avatarPositionX, avatarPositionY, workouts, groups, feed, profiles, onGroups }: { username: string; avatarUrl?: string; avatarPositionX?: number; avatarPositionY?: number; workouts: Workout[]; groups: Group[]; feed: Workout[]; profiles: Map<string, Profile>; onGroups: () => void }) {
...
function CalendarView({ username, avatarUrl, avatarPositionX, avatarPositionY, workouts }: { username: string; avatarUrl?: string; avatarPositionX?: number; avatarPositionY?: number; workouts: Workout[] }) {
  ...
  return <>
    <Header eyebrow="Your consistency" title="Calendar" username={username} avatarUrl={avatarUrl} avatarPositionX={avatarPositionX} avatarPositionY={avatarPositionY} />
...
function GroupsView({ username, avatarUrl, avatarPositionX, avatarPositionY, currentUserId, groups, selectedId, setSelectedId, members, workouts, onManage, onChanged }: { username: string; avatarUrl?: string; avatarPositionX?: number; avatarPositionY?: number; currentUserId: string; groups: Group[]; selectedId: string | null; setSelectedId: (id: string) => void; members: MemberProgress[]; workouts: Workout[]; onManage: () => void; onChanged: () => Promise<void> }) {
  const [membersOpen, setMembersOpen] = useState(false);
  const [section, setSection] = useState<"progress" | "stats">("progress");
  const [working, setWorking] = useState(false);
  const [actionError, setActionError] = useState("");
  const selected = groups.find(group => group.id === selectedId) ?? groups[0];
  if (!selected) return <><Header eyebrow="Accountability together" title="Groups" username={username} avatarUrl={avatarUrl} avatarPositionX={avatarPositionX} avatarPositionY={avatarPositionY} /><EmptyState icon={Users} title="You’re not in a group yet" detail="Create a private group or find one and join with its password or invite code." action="Find or create a group" onAction={onManage} /></>;
  ...
  return <>
    <Header eyebrow={`${groups.length} active group${groups.length === 1 ? "" : "s"}`} title={selected.name} username={username} avatarUrl={avatarUrl} avatarPositionX={avatarPositionX} avatarPositionY={avatarPositionY} />
...
export default function LiftItApp() {
  ...
  return <main className="min-h-screen bg-ink text-white"><div className="mx-auto flex min-h-screen max-w-[1180px]"><DesktopNav view={view} setView={setView} onLog={() => setLogOpen(true)} groupCount={groups.length} /><div className="min-w-0 flex-1"><div className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-white/8 bg-ink/88 px-5 backdrop-blur-xl md:hidden"><Brand /><span className="text-xs font-bold uppercase tracking-[.13em] text-white/35">{pageTitle}</span></div><div className="mx-auto w-full max-w-[720px] px-4 pb-28 pt-7 sm:px-7 md:pb-16 md:pt-10">{view === "home" && <HomeView username={profile.username} avatarUrl={profile.avatar_url} avatarPositionX={profile.avatar_position_x} avatarPositionY={profile.avatar_position_y} workouts={workouts} groups={groups} feed={feed} profiles={profiles} onGroups={() => setView("groups")} />}{view === "calendar" && <CalendarView username={profile.username} avatarUrl={profile.avatar_url} avatarPositionX={profile.avatar_position_x} avatarPositionY={profile.avatar_position_y} workouts={workouts} />}{view === "groups" && <GroupsView username={profile.username} avatarUrl={profile.avatar_url} avatarPositionX={profile.avatar_position_x} avatarPositionY={profile.avatar_position_y} currentUserId={profile.id} groups={groups} selectedId={selectedGroupId} setSelectedId={setSelectedGroupId} members={memberProgress} workouts={groupWorkouts} onManage={() => setGroupOpen(true)} onChanged={() => refresh()} />}{view === "profile" && <ProfileView profile={profile} workoutCount={workouts.length} streak={streak} groupCount={groups.length} onChanged={() => refresh(selectedGroupId ?? undefined)} />}</div></div></div><AppNav view={view} setView={setView} onLog={() => setLogOpen(true)} /><LogDialog open={logOpen} onOpenChange={setLogOpen} onLogged={() => refresh(selectedGroupId ?? undefined)} /><GroupDialog open={groupOpen} onOpenChange={setGroupOpen} discoverGroups={discoverGroups.filter(group => !groups.some(own => own.id === group.id))} onChanged={refresh} /></main>;
}
