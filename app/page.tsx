...
function Header({ eyebrow, title, username, avatarUrl, avatarPositionX, avatarPositionY, avatarZoom }: { eyebrow: string; title: string; username: string; avatarUrl?: string; avatarPositionX?: number; avatarPositionY?: number; avatarZoom?: number }) {
  return <header className="mb-7 flex items-center justify-between"><div><p className="mb-1 text-xs font-bold uppercase tracking-[.16em] text-lime">{eyebrow}</p><h1 className="text-[30px] font-black leading-none tracking-[-.045em] sm:text-4xl">{title}</h1></div><UserAvatar name={username} avatarUrl={avatarUrl} positionX={avatarPositionX} positionY={avatarPositionY} zoom={avatarZoom} /></header>;
}
...
function CalendarView({ username, avatarUrl, avatarPositionX, avatarPositionY, avatarZoom, workouts }: { username: string; avatarUrl?: string; avatarPositionX?: number; avatarPositionY?: number; avatarZoom?: number; workouts: Workout[] }) {
  ...
  return <>
    <Header eyebrow="Your consistency" title="Calendar" username={username} avatarUrl={avatarUrl} avatarPositionX={avatarPositionX} avatarPositionY={avatarPositionY} avatarZoom={avatarZoom} />
...
function GroupsView({ username, avatarUrl, avatarPositionX, avatarPositionY, avatarZoom, currentUserId, groups, selectedId, setSelectedId, members, workouts, onManage, onChanged }: { username: string; avatarUrl?: string; avatarPositionX?: number; avatarPositionY?: number; avatarZoom?: number; currentUserId: string; groups: Group[]; selectedId: string | null; setSelectedId: (id: string) => void; members: MemberProgress[]; workouts: Workout[]; onManage: () => void; onChanged: () => Promise<void> }) {
  ...
  return <>
    <Header eyebrow={`${groups.length} active group${groups.length === 1 ? "" : "s"}`} title={selected.name} username={username} avatarUrl={avatarUrl} avatarPositionX={avatarPositionX} avatarPositionY={avatarPositionY} avatarZoom={avatarZoom} />
    ...
    <Dialog open={membersOpen} onOpenChange={setMembersOpen}><DialogContent className="rounded-[26px] border-white/10 bg-[#181e19] p-5 text-white sm:p-6"><DialogHeader><DialogTitle className="text-2xl font-black">{selected.name} members</DialogTitle><DialogDescription className="text-white/42">{members.length} of 10 spots filled</DialogDescription></DialogHeader><div className="mt-2 space-y-2">{members.map(member => <div key={member.profile.id} className="flex items-center gap-3 rounded-2xl bg-white/5 p-3"><UserAvatar name={member.profile.username} avatarUrl={member.profile.avatar_url} positionX={member.profile.avatar_position_x} positionY={member.profile.avatar_position_y} zoom={member.profile.avatar_zoom} /><div className="min-w-0 flex-1"><p className="truncate font-bold">{member.profile.username}{member.profile.id === currentUserId && <span className="ml-2 text-xs font-medium text-white/35">you</span>}</p>{member.role === "owner" ? <p className="flex items-center gap-1 text-xs font-bold text-amber-300"><Crown className="size-3.5 fill-amber-300/20" />Owner</p> : <p className="text-xs capitalize text-white/40">Member</p>}</div></div>)}</div></DialogContent></Dialog>
  </>;
}
